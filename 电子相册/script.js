...
function viewportScale() {
  ...
  function heart(card, n, spin) {
    ...
    const bob = Math.sin(spin * 2.4 + card.index * 0.72) * 6 + alternate * 9;
    const drift = Math.cos(spin * 1.3 + card.index * 0.93) * 4;
    return {
      x: slot[0] * 1.42 + drift,
      y: slot[1] * 1.28 + 6 + bob,
      z: 40 + card.index * 1.2,
      rx: 0,
      ry: 0,
      rz: Math.sin(card.index * 0.82 + card.seed * 3) * 6,
      s: 0.7 + card.seed * 0.04,
      o: 0.98
    };
  }
  ...
  function wave(card, n, phase, spin) {
    ...
    const order = n > 1 ? card.index / (n - 1) : 0;
    const revealStart = order * 0.68;
    const revealSpan = 0.28;
  ...
  function ring(card, n, spin) {
    const angle = (card.index / n) * Math.PI * 2 + spin;
    const pulse = Math.sin(angle * 3) * 18;
    const uprightTilt = Math.sin(angle * 1.35 + card.seed * 5) * 8 + (card.seed - 0.5) * 7;
    return {
      x: Math.cos(angle) * (398 + pulse),
      y: Math.sin(angle) * 202 - 4,
      z: Math.sin(angle) * 70,
      rx: 0,
      ry: Math.cos(angle) * 3.5,
      rz: uprightTilt,
      s: 0.86,
      o: 0.98
    };
 ...
 function finalRing(card, n) {
  ...
  function applyCard(card, t, p, n) {
    ...
    if (!card.isDragging && card.dragPhase !== null && phase !== card.dragPhase) {
      card.dragX *= 0.86;
      card.dragY *= 0.86;
